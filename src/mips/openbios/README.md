...
- `BOARD=zn1` or `BOARD=zn2` will build a kernel that can run on the Sony ZN-1 or ZN-2 arcade boards respectively. This option is only useful alongside `EMBED_PSEXE`, as it will force `BOOT=rom` and `BOOT_MODE=psexe`; unlike the official ZN BIOSes, OpenBIOS does *not* implement game decryption and booting in the kernel.
...
The startup point for the ROM version is the function `_reset` located in [`boot/psx.s`](boot/psx.s) (or [`system573.s`](boot/system573.s), [`zn1.s`](boot/zn1.s) or [`zn2.s`](boot/zn2.s) depending on which board the kernel is compiled for). The rom cart version is the function `_cartBoot` in the same file.
